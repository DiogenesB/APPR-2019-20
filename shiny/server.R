...
shinyServer(function(input, output) {
  output$zemljevid <- renderPlot(
    ...
    geom_point(aes(x=zemljepisna.dolzina, y=zemljepisna.sirina, color=tip.placila), data = distinct(select(lokacija_kupcev, kljuc.uporabnika, zemljepisna.sirina, zemljepisna.dolzina, tip.placila), kljuc.uporabnika, .keep_all = TRUE) %>% filter(tip.placila==input$tip)) + 
